...
#' @description
#'
#' These functions offer relaxed matching of one vector in another.  In contrast
#' to the similar [`base::match()`] and [`base::%in%`] functions they just accept `numeric`
#' arguments but have an additional `tolerance` argument that allows relaxed
#' matching.
#'
#' @param x `numeric`, the values to be matched. In contrast to
#'     [`base::match()`] `x` has to be sorted in increasing order and must not
#'     contain any `NA`.
#' @param table `numeric`, the values to be matched against. In contrast to
#'     [`base::match()`] `table` has to be sorted in increasing order and must not
#'     contain any `NA`.
#' @param tolerance `numeric`, accepted tolerance. Could be of length one or the
#'     same length as `x`.
...
#' @param nomatch `integer(1)`, if the difference between the value in `x` and
#'     `table` is larger than `tolerance` `nomatch` is returned.
#' @param .check `logical(1)` turn off checks for increasingly sorted `x` and
#'     `table`. It also disables most other input validation checks.  This
#'     should just be done if it is ensured by other methods that `x` and
#'     `table` are sorted, see details.
#'
#' @details For `closest`/`common` the `tolerance` argument could be set to `0`
#'     to get the same results as for [`base::match()`]/[`base::%in%`]. If it is
#'     set to `Inf` (default) the index of the closest values is returned
#'     without any restriction.
#'
#' It is not guaranteed that there is a one-to-one matching for neither the `x`
#' to `table` nor the `table` to `x` matching.
#'
...
#' This behaviour is identical to [`base::match()`]. For `duplicates="closest"`
#' just the closest element in `x` gets the corresponding index in `table` and
...
#' is already returned for another `x` with a smaller difference to this `index`
#' the higher one is returned for `duplicates = "closer"` (but only if there is
#' no other `x` that is closer to the higher one).  For `duplicates="remove"`
#' all multiple matches are returned as `nomatch` as above.
#'
...
#' compare each element against its precursor.  Depending on the length and
#' distribution of `x` and `table` these checks take equal/more time than the
#' whole `closest` algorithm. If it is ensured by other methods that both
#' arguments `x` and `table` are sorted the tests could be skipped by `.check =
#' FALSE`. In the case that `.check = FALSE` is used and one of `x` and `table`
#' is not sorted (or decreasingly sorted) the output would be incorrect in the
#' best case and result in infinity loop in the average and worst case.
#'
...
#' element in `x` was found in `table`. It is similar to [`base::%in%`].
#' @aliases common
#' @seealso [`base::%in%`]
#' @export
#' @examples
