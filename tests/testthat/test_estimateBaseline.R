...
test_that("estimateBaselineConvexHull", {
    b <- c(10:1, rep(1, 10))
    expect_identical(MsCoreUtils:::.estimateBaselineConvexHull(s$mass,
                                                               s$intensity), b)
    ## user method
...
test_that("estimateBaselineMedian", {
    ...
    expect_identical(MsCoreUtils:::.estimateBaselineMedian(s$mass, s$intensity,
                                                           1), b1)
    expect_identical(MsCoreUtils:::.estimateBaselineMedian(s$mass, s$intensity,
                                                           2), b)
    ## user method
...
test_that("estimateBaselineSnip", {
    ...
    for (j in seq(along = b)) {
        ...
        expect_identical(MsCoreUtils:::.estimateBaselineSnip(s$mass, s$intensity,
                                                            2, decreasing=d),
                         b[[j]]$b)
        expect_equal(MsCoreUtils:::.estimateBaselineSnip(s$mass, s$intensity,
                                                        decreasing=d),
                     b[[j]]$b100)
