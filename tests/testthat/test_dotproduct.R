test_that("dotproduct", {
    ...
    expect_error(dotproduct(x = x, y = "a"), "matrix")
    expect_error(dotproduct(x = "a", y = y), "matrix")
    ...
    expect_error(dotproduct(x, y), "'mz' and 'intensity'")
    colnames(x) <- c("mz", "foo")
    colnames(y) <- c("mz", "intensity")
    expect_error(dotproduct(x, y), "'mz' and 'intensity'")
    ...
    y <- matrix(c(1:3, 1:3), ncol = 2)
    colnames(y) <- c("mz", "intensity")
    expect_error(dotproduct(x, y), "matrix")
    ...
    y <- matrix(c(1:3, 1:3), ncol = 2)
    colnames(x) <- colnames(y) <- c("mz", "intensity")
    expect_error(dotproduct(x, y), "'numeric' matrix")
    x <- matrix(c(1:3, 1:3), ncol = 2)
