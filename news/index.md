...
- [`impute_mixed()`](https://rformassspectrometry.github.io/MsCoreUtils/reference/imputation.md):
  new `split` argument that controls of imputed values are computed from
  the whole matrix of MAR and MNAR sub-matrices (see [issue
...
- Fix bug in
  [`impute_mixed()`](https://rformassspectrometry.github.io/MsCoreUtils/reference/imputation.md)
  and allow for two MARGINs for mixed imputation. (see [issue
  139](https://github.com/rformassspectrometry/MsCoreUtils/issues/139)).
...
- Add parameter `matchedPeaksCount` to spectra similarity/distance
  functions to report, in addition to the similarity, also the number of
  matched peaks.

### MsCoreUtils 1.23.2

- Fix *Found non-API call to R: ‘SETLENGTH’* in `C_reduce`,
  `C_join_inner`, and `C_join_outer` by using `lengthgets()` instead
  (see [issue
  136](https://github.com/rformassspectrometry/MsCoreUtils/issues/136)).
...
- Fix
  ...
  to merge also ranges with the same end.

### MsCoreUtils 1.19.1

- Add new
  [`reduce()`](https://rformassspectrometry.github.io/MsCoreUtils/reference/reduce.md)
  function to *reduce* overlapping numeric ranges to disjoint ranges.
...
- Fix
  ...
  to not return also the file name if the input parameter contains only
...
- Fix partial argument match (see issue
  [\#125](https://github.com/RforMassSpectrometry/MsCoreUtils/issues/125)).
...
- Check that package is available and namespace is loaded adding
  [`stopifnot()`](https://rdrr.io/r/base/stopifnot.html) when calling
  [`requireNamespace()`](https://rdrr.io/r/base/ns-load.html).

### MsCoreUtils 1.11.5

- Add function `maxi` to determine the maximal intensity value. This
  function returns `NA_real_` instead of `-Inf` if all values are
  missing or if the length of the input parameter is 0.

### MsCoreUtils 1.11.4

- Check if parameter `y` is increasingly ordered in `bin`:
  [issue](https://github.com/rformassspectrometry/MsCoreUtils/issues/108)
  [\#108](https://github.com/RforMassSpectrometry/MsCoreUtils/issues/108).

### MsCoreUtils 1.11.3

- Add function `sumi` to sum intensity values with *correct* NA
  handling.

### MsCoreUtils 1.11.2

- Reimplement `between` in C (see issue
  [\#105](https://github.com/RforMassSpectrometry/MsCoreUtils/issues/105)).
- Use symbols to call registered C methods for faster lookup (see PR
  [\#106](https://github.com/RforMassSpectrometry/MsCoreUtils/issues/106)
  and [Writing R extensions: Converting a package to use
...
- New `impute_mle2()` function that uses `norm2` (see issue
  [\#100](https://github.com/RforMassSpectrometry/MsCoreUtils/issues/100)).
...
- Function `bin` gains parameter `returnMids` to choose whether or not
  bin mid-points should be returned in the result `list`.
...
- Use
  [`Matrix::colSums()`](https://rdrr.io/pkg/Matrix/man/colSums-methods.html)
...
- New
  ...
  that uses an adjacency matrix to aggregate quantitative features.
- Set colnames to the outputs of
  [`aggregate_by_matrix()`](https://rformassspectrometry.github.io/MsCoreUtils/reference/aggregate.md)
  and
  [`aggregate_by_vector()`](https://rformassspectrometry.github.io/MsCoreUtils/reference/aggregate.md)
  to make sure that these are always set and not reply on the underlying
  function.
...
- Rewrite `c("left", "right", "inner", "outer")` `join` in C
  \<2020-10-06 Tue\>.
...
- Fix
  [\#65](https://github.com/rformassspectrometry/MsCoreUtils/issues/65)
  and
...
- Ensure `closest` accept just argument `tolerance` of length 1 or
  `length(x)`; see also
  [\#61](https://github.com/rformassspectrometry/MsCoreUtils/issue/61),
  [PR](https://github.com/rformassspectrometry/MsCoreUtils/pull/62)
  [\#62](https://github.com/RforMassSpectrometry/MsCoreUtils/issues/62)
  \<2020-08-07 Thu\>.
...
- For an empty `table` `closest` and `common` return a vector of length
  `x` with `NA` or `FALSE`, respectively (instead of `1` and `TRUE`).
  Fixes
  [\#55](https://github.com/rformassspectrometry/MsCoreUtils/pull/55)
  \<2020-06-18 Thu\>.
...
- Add some popular distance/similarity metrices: `ndotproduct`
  `neuclidean` `navdist` `nspectraangle`; see also
  [PR](https://github.com/rformassspectrometry/MsCoreUtils/pull/33)
  [\#33](https://github.com/RforMassSpectrometry/MsCoreUtils/issues/33).
